...
<%	Dim justi_sTitle, justi_oJustificationMsg, justi_oJustification, justi_oJustificationList, justi_row
	Dim field_id, justlen,dlgName
    Dim FrmType, dlgNameTransId,strMsg1_Trans, dlgInstructions
    ...
    FrmType = GetLongAsStr(Request.QueryString("fType"))
    If FrmType = "1" Then
        dlgName = GetTranslationData("E2B_REJECT_FU")
        dlgNameTransId  = "E2B_REJECT_FU"
        dlgInstructions = GetTranslationData("REJECT_FU_E2B_CASE_INSTRUCTIONS")
        strMsg1_Trans   = GetTranslationData("E2B_REJECT_FU")
    Else
        dlgName =GetTranslationData("REJECT_NULLIFICATION")
        dlgNameTransId  = "REJECT_NULLIFICATION"
        dlgInstructions = GetTranslationData("REJECT_NULLIFICATION_E2B_CASE_INSTRUCTIONS")
        strMsg1_Trans   = GetTranslationData("E2B_NULL_DECLINED")
    End If
%>
<head>
    <!-- Page Title -->
...
var l_attempt = 0;
var selected = 0;
var sE2BDeckineTitle = <%=JavaScriptSanitize(strMsg1_Trans) %>;
...
async function fn_password_verify()
{
    ...
    {
        bPasswordOK = false;
        await MessageBoxRes("GENERAL_WARNING", sE2BDeckineTitle, sErrMsg);
...
async function fn_ClickOK()
{
    ...
    if (document.all.ReAuth.value.length == 0) {
	    await MessageBoxRes("RE_AUTH_JUSTIFICTN_PROVIDE_PASS",sE2BDeckineTitle);
	    return -1;
	}
	<%Else %>
    if (s_password.length == 0) {
	    await MessageBoxRes("JUSTIFICTN_PROVIDE_PASS",sE2BDeckineTitle);
	...
	if (s_notes.length == 0) {
		await MessageBoxRes("ACCEPT_CASE_NOTES_REQ",sE2BDeckineTitle);
	...
	if (!bPasswordOK)
	{
        ...
        await MessageBoxRes("E2B_REJECTCASE_ATTMPT_EXCEED", sE2BDeckineTitle);
...
<body>
    <!-- Outer Box Starts -->
    <table class="table border-blue inner-table" width="100%" height="100%" cellspacing="0"
        cellpadding="5">
        <tr height="25px">
            <td class="section-header-middle">
                <% BuildLabelDirect(GetTranslationData(dlgNameTransId)).SetStyleSheet("label label-section").Render()%>
            </td>
        </tr>
        <tr>
            <td class="padding-all">
                <table class="table inner-table" width="100%" height="100%" cellspacing="0">
                    <col width="26%" />
                    <col />
                    <tr height="55px">
                        <td class="page-text" colspan="2">
                            <%BuildLabelDirect(dlgInstructions).Render()%>
                        </td>
                    </tr>
                    <tr height="20px">
                        <td>
