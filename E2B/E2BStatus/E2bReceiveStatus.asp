...
<%      
	...
	Dim oDateRangeMsg
	Dim ErrorNum, Error
	Dim Agency, DateFrom, DateTo, MsgFrom, MsgTo, MsgType, oContact
	Dim oOutMsg, oRecList, oRec, lNumRec, lContactRec, oContactMsg, oContactList
	...
	Dim oDateRangeList, oDateRange
	Dim DateRange, sSortString
	...
	MsgFrom = GetString(GetRequest("MessageFrom"), "")
	MsgTo = GetString(GetRequest("MessageTo"), "")
	        ...
	        <table class="table" cellpadding="0" cellspacing="0" width="100%" height="100%">
		        <tr style="height: 25px">
			        <td class="padding-left-right">
				        <%BuildLabelDirect("ICSR Receive Status").SetStyleSheet("label label-page-header").Render() %>
			        </td>
		        </tr>
		        <tr style="height: 25px">
			        <td>
				        <!--#INCLUDE VIRTUAL="/E2B/E2BStatus/Report_Filter_inc.asp" -->
			        </td>
		        </tr>
		        <tr style="height: 5px">
			        <td></td>
		        </tr>
		        <!--#INCLUDE VIRTUAL="/E2B/E2BStatus/Result_inc.asp" -->
...
Function GetResults()
	...
	ElseIf RadioBtn = 1 Then
		Call SetXMLValueDirect (oMessage, "RPT_E2B_MESSAGE_FROM", cfCmn_FindRegEx(MsgFrom, "0-9"))
		Call SetXMLValueDirect (oMessage, "RPT_E2B_MESSAGE_TO", cfCmn_FindRegEx(MsgTo, "0-9"))
	End If		
	...
	WriteMessageToCache "E2BRESULTSET", oSearchResults, true
	DateFrom = dtDateFrom
	DateTo = dtDateTo
	GetResults = True
End Function
...
Function Rpt_InitializeSearchResultsCache()	    
	...
	Rpt_sHtmlTemplate = ReadTextFromFile("E2BRECEIVESTATUS.tpl")	
	...
	Rpt_oGridControl.SetDataSource GetMessageCacheFileName("E2BRESULTSET", true), "RPT_E2B"    		
