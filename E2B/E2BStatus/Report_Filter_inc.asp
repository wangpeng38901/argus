					...
					<tr>
						<td class="no-padding-left">
							<table class="table inner-table" cellspacing="2" cellpadding="2" width="100%">
								...
								<col width="20%" />
								<tr>
									<td><%BuildLabelDirect("Agency/Trading Partner ").Render()%></td>
									<td colspan="5">
										<%
								...
								<tr>
									<td>
										...
										<%BuildLabelDirect("Receive Date Range From").SetStyleSheet("label label-no-padding").Render()%>
									</td>
									<td>
									...
									<td>
										<%BuildListDirectFromXML("RangeList",RangeList, false, 5, oDateRangeList, "LM_DATE_RANGES_RANGE_ID", "LM_DATE_RANGES_RANGE_NAME").AddBtmOption("Custom Date Range:-1").onchange("fn_ChangeDateRange(this.value)").Style("width:100%").Render() %>
									</td>
									<td>&nbsp;
									</td>
								</tr>
								<tr>
									...
									<td>
										<%BuildControlDirect(CTL_TEXTBOX_SIMPLE, "MessageFrom", MsgFrom, false, 7, "").SetMaxLength(100).Style("width:100%").Render()%>
									</td>
									<td>
										<%BuildLabelDirect("To ").Render() %>
									</td>
									<td>
										<%BuildControlDirect(CTL_TEXTBOX_SIMPLE, "MessageTo", MsgTo, false, 8, "").SetMaxLength(100).Style("width:100%").Render()%>
									</td>
									<td>
										<%BuildLabelDirect("Type ").Render() %>
									</td>
									<td>
										<select class="ddlist" style="width: 100%;" name="MessageType" onchange="fn_ChangeTypeRange()" tabindex="9">
...
<script type="text/javascript">
	...
	function fn_GetData()
	{
		...
		document.Frm_E2B.action = "E2bReceiveStatus.asp?bsearch=1" + "&RadioBtn=" + document.Frm_E2B.RadioBtn.value + "&DateFrom=" + document.Frm_E2B.DateFrom.value + "&DateTo=" + document.Frm_E2B.DateTo.value + "&rangelist=" + document.Frm_E2B.RangeList.value + "&typelist=" + document.Frm_E2B.TypeList.value + "&MessageFrom=" + fn_URLEncode(document.Frm_E2B.MessageFrom.value) + "&MessageTo=" + fn_URLEncode(document.Frm_E2B.MessageTo.value) + "&OffSet=" + document.Frm_E2B.OffSet.value  + "&GmtOffSet=" + document.Frm_E2B.GmtOffSet.value + "&<%=GetRequestKeyValue()%>";
	...
	async function fn_OpenAckStatus(temp)
	{
		var strURL;
		strURL = "/E2B/E2bStatus/MsgAckStatus.asp?AckType=" + "RECEIVE" + "&MessageNumb=" + temp + "&GmtOffSet=" + document.Frm_E2B.GmtOffSet.value;
